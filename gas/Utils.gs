 ...
 * @param {number} col - 列番号
 */
function testMergedCellProcessing(sheet, row, col) {
  try {
    console.log(`🧪 結合セル処理テスト: 行${row}列${getColumnLetter(col)}`);
    
    // 通常の値取得
    const normalValue = sheet.getRange(row, col).getValue();
    console.log(`  通常値: "${normalValue}"`);
    
    // 結合セル判定
    const isMerged = isMergedCell(sheet, row, col);
    console.log(`  結合セル判定: ${isMerged}`);
    ...
    if (isMerged) {
      ...
      if (mergedRanges.length > 0) {
        ...
        console.log(`  結合範囲: 行${range.getRow()}-${range.getLastRow()}, 列${getColumnLetter(range.getColumn())}-${getColumnLetter(range.getLastColumn())}`);
      }
      
      // 統合処理テスト
      const mergedValue = getMergedCellValueWithMergeInfo(sheet, row, col);
      console.log(`  統合後値: "${mergedValue}"`);
    }
